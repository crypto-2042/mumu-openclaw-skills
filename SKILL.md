...
- **To Create a Brand New Novel**:
  `python -m mumu_agent.bind_project --action create --title "<Title>" --description "<Plot>" --theme "<Theme>" --genre "<Genre>"`
...
- **To View Existing Novels**:
  `python -m mumu_agent.bind_project --action list`
  
- **To Take Over an Existing Novel**:
  `python -m mumu_agent.bind_project --action bind --project_id <The ID>`
...
`python -m mumu_agent.trigger_batch --count <Number of Chapters>`
...
`python -m mumu_agent.analyze_chapter --chapter_id <Chapter ID>`
...
2. `python -m mumu_agent.review_chapter --action rewrite --chapter_id <Chapter ID> --file rewrite.md`
...
`python -m mumu_agent.review_chapter --action approve --chapter_id <Chapter ID>`

### 5. Add Foreshadowing & Memory (Lore Injection)
Proactively lay down plot devices for the future:
`python -m mumu_agent.manage_memory  --action add_foreshadow --content "<Lore or foreshadowing text>"`
